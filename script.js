document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetAndLoadMovies(shouldScroll = false) {
        if (shouldScroll) {
        ...
        // [FIX] Only shuffle the array if the sort order is "Default"
        if (sortBy === 'default') {
            moviesToConsider = shuffleArray(moviesToConsider);
        }
        ...
        // [FIX] Perform other sorting only if not default
        if (sortBy !== 'default') {
            switch (sortBy) {
    ...
    async function initializeApp() {
        ...
        resetAndLoadMovies(true);
        if (logoLink) { logoLink.addEventListener('click', (e) => { e.preventDefault(); if (searchInput) searchInput.value = ''; if (sortBySelect) sortBySelect.value = 'default'; selectedHeroIdHidden.value = ""; selectedHeroNameDisplay.textContent = "All Hero Movies"; if (heroOptionsPanel.classList.contains('open')) populateHeroOptions(allHeroes, '', ''); selectedYearHidden.value = ""; selectedYearNameDisplay.textContent = "Filter by Year"; yearOptionsSearchInput.value = ""; if (yearOptionsPanel.classList.contains('open')) populateYearOptions(allAvailableYears, '', ''); resetAndLoadMovies(true); }); }
        if (searchInput) {
            searchInput.addEventListener('input', () => { clearTimeout(searchTimeout); searchTimeout = setTimeout(() => resetAndLoadMovies(false), SEARCH_DEBOUNCE_MS); });
        ...
        if (sortBySelect) sortBySelect.addEventListener('change', () => resetAndLoadMovies(true));
