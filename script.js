...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Constants ---
    const MOVIES_STORAGE_KEY = 'teluguMovies_movies_v10_youtubeThumb_r1'; // Updated key for new logic
    const HEROES_STORAGE_KEY = 'teluguMovies_heroes_v10_youtubeThumb_r1'; // Updated key
    ...
    function processMovieData(moviesArray) {
        ...
        return moviesArray.map(movie => ({
            ...
            thumbnailUrl: movie.thumbnailUrl || "", // Default to empty string, logic in createMovieItemElement will handle it
            heroId: movie.heroId || null,
            redirectUrl: movie.redirectUrl || '#',
            year: movie.year ? parseInt(movie.year, 10) : 0
        })).filter(movie => movie.redirectUrl); // Only require redirectUrl now, thumbnail can be derived
    }
    ...
    function getYouTubeThumbnailUrl(videoID, quality = 'mqdefault') {
        // mqdefault (320x180), hqdefault (480x360), sddefault (640x480), maxresdefault (1920x1080 if available)
    ...
    function createMovieItemElement(movieData) {
        ...
        let finalThumbnailUrl = movieData.thumbnailUrl; // Start with provided URL

        // If no specific thumbnail, or it's empty, and redirectUrl is a YouTube video, try to use YouTube's thumbnail
        if ((!finalThumbnailUrl || finalThumbnailUrl.trim() === "") && movieData.redirectUrl) {
            ...
            if (youtubeVideoID) {
                ...
                if (youtubeThumb) {
                    finalThumbnailUrl = youtubeThumb;
                }
            }
        }
        // If still no valid thumbnail, use the global placeholder
        if (!finalThumbnailUrl || finalThumbnailUrl.trim() === "") {
            finalThumbnailUrl = placeholderThumb;
        }
    ...
    async function initializeApp() {
        ...
        if (yearOptionsSearchInput) { yearOptionsSearchInput.addEventListener('input', () => {clearTimeout(yearSearchTimeout); yearSearchTimeout = setTimeout(() => {const typedText = yearOptionsSearchInput.value.trim(); populateYearOptions(allAvailableYears, typedText, selectedYearHidden.value); }, YEAR_SEARCH_DEBOUNCE_MS);}); yearOptionsSearchInput.addEventListener('keydown', (e) => { if (e.key === 'Escape') { e.stopPropagation(); closeYearOptionsPanel(); yearSelectTrigger.focus(); } else if (e.key === 'Enter') { e.preventDefault(); const options = Array.from(yearOptionsList.children).filter(li => !li.classList.contains('no-year-results')); if (currentFocusedYearOptionIndex >= 0 && currentFocusedYearOptionIndex < options.length) { options[currentFocusedYearOptionIndex].click(); } else if (options.length > 0) { options[0].click(); } else { const typedYear = yearOptionsSearchInput.value.trim(); if (typedYear === "" || /^\d{1,4}$/.test(typedYear)) { selectedYearHidden.value = typedYear; selectedYearNameDisplay.textContent = typedYear === "" ? "Filter by Year" : typedYear; closeYearOptionsPanel(); resetAndLoadMovies(); }}} else if (e.key === 'ArrowDown') { e.preventDefault(); const firstValidOption = yearOptionsList.querySelector('li:not(.no-year-results)'); if (firstValidOption) { yearOptionsList.focus(); if (currentFocusedYearOptionIndex < 0 || !yearOptionsList.children[currentFocusedYearOptionIndex] || yearOptionsList.children[currentFocusedYearOptionIndex].classList.contains('no-year-results')) { currentFocusedYearOptionIndex = Array.from(yearOptionsList.children).indexOf(firstValidOption); }  updateFocusedYearOption(); }}}); }
        if (yearOptionsList) { yearOptionsList.addEventListener('keydown', (e) => { const options = Array.from(yearOptionsList.children).filter(li => !li.classList.contains('no-year-results')); if (!options.length) return; let newIndex = currentFocusedYearOptionIndex; if (e.key === 'ArrowDown') { e.preventDefault(); newIndex = (currentFocusedYearOptionIndex + 1) % options.length; } else if (e.key === 'ArrowUp') { e.preventDefault(); newIndex = (currentFocusedYearOptionIndex - 1 + options.length) % options.length; } else if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); if (newIndex >= 0 && newIndex < options.length) { options[newIndex].click(); } return; } else if (e.key === 'Escape') { closeHeroOptionsPanel(); yearSelectTrigger.focus(); return; } else if (e.key === 'Home') { e.preventDefault(); newIndex = 0; } else if (e.key === 'End') { e.preventDefault(); newIndex = options.length - 1; } else if (e.key.length === 1 && e.key.match(/[a-z0-9]/i)) { yearOptionsSearchInput.focus(); return; } if (newIndex !== currentFocusedYearOptionIndex) { currentFocusedYearOptionIndex = newIndex; updateFocusedYearOption(); } }); }
